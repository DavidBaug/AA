...
# Variables globales para los distintos ejercicios
...
def ejercicio1_1():
    ...
    # Muestras divididas en dos variables    
    x = unif[:,0]
    y = unif[:,1]
...
# Función que nos permite encontrar un modelo usando el algoritmo del perceptron
...
# Error de clasificación, porcentaje de fallos
...
# Función para alterar nuestro modelo
def gradient(w, X, y):
    return ((-y*X)/(1+np.exp(y*X.dot(w))))/X.shape[0]


# Porcentaje de aciertos
def aciertos_logistica(w, X, y):
    aciertos = 0
    ...
    print('Porcentaje de éxito = {}{}'.format((aciertos/X.shape[0])*100,'%'))
...
def ejercicio2_2():
    ...
    # Etiquetamos nuestra muestra con la recta calculada
    w = aniade_etiquetas([a,b], X[:,0],X[:,1])
    
    # Variable para pintar los puntos de distinto color en el plot
    label = w.copy()
    label = label[:,2]
    label[(label == -1)] = 0
    ...
    X = w.copy()
    X[:,2] = 1          # Añadimos una columna de 1 para calcular el término independiente
    
    y = w.copy()
    y = y[:,2]
    
    # Modelo que usaremos en la regresión
    model = np.array([0,0,0],np.float64)
    ...
    #Muestra de 1234 puntos
    X = simula_unif(N=1234, size=(0,2))
    
    print('Generadas 1234 muestras de una distribución uniforme.\n')
    
    # Etiquetamos el nuevo conjunto con la recta anterior
    w = aniade_etiquetas([a,b], X[:,0],X[:,1])
    
    # Colores etiquetas plot
    label = w.copy()
    label = label[:,2]
    label[(label == -1)] = 0
    ...
    plt.clf()
    plt.title('Clasificación 1000 muestras')
    plt.plot(range(3), [ a*i+b for i in range(3)], 'k')
    plt.axis([np.min(w[:,0]),np.max(w[:,0]),np.min(w[:,1]),np.max(w[:,1])])
    plt.scatter(w[:,0], w[:,1], c=label, cmap='rainbow_r', edgecolor='white')
    plt.show()

    print('Aplicamos nuestro modelo calculado anteriormente a la nueva muestra.\n')    
    ...
    plot_data(X,y,model, 'Modelo regresión con 1234 muestras')
